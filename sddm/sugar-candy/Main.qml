...
Pane {
    ...
    // Login form only on the centre monitor (X11 name under the greeter's
    // X server). Guard: if no screen has that name (driver/name change),
    // show the form on every screen rather than none -> no lockout.
    readonly property string formScreenName: "DisplayPort-1"
    readonly property bool formScreenPresent: {
        var scr = Qt.application.screens;
        for (var i = 0; i < scr.length; i++)
            if (scr[i].name === formScreenName) return true;
        return false;
    }
    readonly property bool showForm: !formScreenPresent || Screen.name === formScreenName
    ...
    Item {
        ...
        Rectangle {
            ...
            visible: root.showForm && config.HaveFormBackground == "true"
        ...
        Button {
            ...
            visible: root.showForm && virtualKeyboard.status == Loader.Ready && config.ForceHideVirtualKeyboardButton == "false"
        ...
        ShaderEffectSource {
            ...
            visible: root.showForm && (config.FullBlur == "true" || config.PartialBlur == "true")
        }
